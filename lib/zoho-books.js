...
const itemNameAliases = {
  ...
  "never miss a lead": "lead follow up crm setup",
  "crm automation": "lead follow up crm setup",
  "qr code campaigns": "qr code campaign setup",
  "storefront sign": "storefront signs",
  "storefront sign and window graphics": "storefront signs",
};

function itemMatchKey(value) {
  const normalized = normalizeItemName(value);
  return itemNameAliases[normalized] || normalized;
}

async function getZohoItemForOrder(order) {
  const productName = order.product_name || order.productName;
  if (!productName) return null;

  const data = await zohoFetch("items?per_page=200");
  const items = data.items || [];
  const target = itemMatchKey(productName);

  return (
    items.find((item) => item.status === "active" && itemMatchKey(item.name) === target) ||
    null
  );
}
...
async function buildLineItem(order) {
  const quantity = Number(order.quantity || 1);
  const rate = Number(order.unit_rate || order.unitRate || 0);
  const zohoItem = await getZohoItemForOrder(order);
  ...
  if (zohoItem?.item_id) {
    lineItem.item_id = zohoItem.item_id;
  }
...
async function createEstimate(order, contactId) {
  const payload = {
    ...
    line_items: [await buildLineItem(order)],
...
export async function syncOrderToZohoBooks(order, mode = "estimate") {
  ...
  return {
    ...
    zohoItemId: document?.line_items?.[0]?.item_id,
    zohoItemName: document?.line_items?.[0]?.name,
