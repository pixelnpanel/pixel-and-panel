...
export const cityServiceServices = {
  'vehicle-graphics': {
    ...
    localContext: (city) => `In ${city.name}, most customers see your vehicle before they ever visit your website or find your business card. Contractors working ${city.neighborhoods}, delivery drivers covering ${city.region}, and service teams running calls across ${city.highways} all have one thing in common: the vehicle is the first impression. A professionally lettered truck projects credibility before you knock on the door. A wrapped fleet tells customers you're an established local business, not a one-person operation working out of a personal car. For ${city.businessTypes}, vehicle graphics are often the most cost-effective marketing spend available — reaching new potential customers every mile, every day, at no recurring cost after the initial install.`,
  ...
  'storefront-signs': {
    ...
    localContext: (city) => `${city.name} businesses compete for attention ${city.landmark} — areas where potential customers drive past dozens of options before deciding where to stop. A weak or missing storefront sign doesn't just hurt visibility; it signals that a business isn't established enough to invest in its own location. For ${city.businessTypes}, a professional exterior sign is the single most visible piece of marketing you own. It works 24 hours a day, 7 days a week, and unlike digital ads, it never goes dark when your budget runs out. Most ${city.name} commercial corridors also have sign regulations set by the ${city.permitOffice} — knowing those requirements before you order saves time and avoids costly revisions.`,
  ...
  'web-development': {
    ...
    localContext: (city) => `Most customers in ${city.name} check a business's website before they call — even if they found you through a sign, a referral, or a Google search. If your website is slow, hard to read on a phone, or missing basic information like your services, service area, and contact options, you're losing customers before they ever reach out. For ${city.businessTypes} in ${city.name}, the website doesn't need to be complicated. It needs to be fast, clear, and designed around one goal: making it easy for a local customer to take the next step. That means a mobile-first layout, a visible phone number, a quote or contact form, and service pages that explain what you do for people in ${city.name} specifically.`,
    ...
    process: [
      ...
      ['Design and content', 'We build a clean, mobile-first layout with your brand colors, logo, and service information. You review and request changes.'],
      ['Launch with SEO foundation', 'Every site ships with proper metadata, local signals, and a Google-ready page structure. You don\'t pay extra for the basics.'],
