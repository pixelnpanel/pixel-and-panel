export const cityServiceCities = {
  ...
  // Houston runs as a service-area market: no published address anywhere, and
  // coverage is expressed through areaServed. Four services are deliberately
  // NOT generated here because a hand-written /houston/<service> page already
  // owns that query — see cityServiceHasDedicatedPage below.
  'houston-tx': {
    name: 'Houston',
    state: 'TX',
    slug: 'houston-tx',
    // Houston's hub is /houston, not /service-area/houston-tx — that page does
    // not exist and must not, since it would duplicate the hand-written hub.
    hubHref: '/houston',
    region: 'the Greater Houston area',
    context:
      'Houston is the largest city in Texas and has no zoning, so competing businesses sit side by side along the same commercial corridors and signage is often the only thing separating them.',
    businessTypes: 'contractors, restaurants, retail shops, medical offices, and industrial service companies',
    landmark: 'along the Katy Freeway, Westheimer Road, and the Beltway 8 commercial corridors',
    highways: 'I-10, I-45, US-59/I-69, Loop 610, and Beltway 8',
    neighborhoods: 'Downtown, The Heights, Montrose, Uptown/Galleria, the Energy Corridor, and Westchase',
    permitOffice: 'City of Houston Public Works',
  },
  'port-arthur-tx': {
    name: 'Port Arthur',
    state: 'TX',
...
export const cityServiceServices = {
  'vehicle-graphics': {
    ...
    seo: {
      'houston-tx': {
        title: 'Vehicle Wraps Houston TX | Truck Lettering & Fleet Graphics',
        description: 'Custom truck lettering, vehicle wraps, magnets, and fleet graphics for Houston businesses. Durable vinyl built for Gulf Coast sun. Free quote.',
      },
      'beaumont-tx': {
  ...
  'storefront-signs': {
    ...
    seo: {
      'houston-tx': {
        title: 'Storefront Signs Houston TX | Custom Business Signs',
        description: 'Custom storefront and building signs for Houston businesses. Houston has no zoning but still permits signs — we handle sizing, material, and layout. Free quote.',
      },
      'port-arthur-tx': {
  ...
  'metal-signs': {
    ...
    seo: {
      'houston-tx': {
        title: 'Metal Signs Houston TX | Aluminum & Outdoor Business Signs',
        description: 'Custom aluminum and metal signs for Houston businesses. Rust-free, UV-stable, and built to survive Gulf Coast heat and storm season. Free quote.',
      },
      'beaumont-tx': {
  ...
  'web-development': {
    ...
    // Beaumont title and H1 must not carry the same phrasing. The title owns
    // "web design beaumont tx" (95 impressions, the biggest single query);
    // the H1 covers the "website design" cluster the title cannot reach —
    // "website design beaumont tx", "beaumont texas website design", and
    // "website design beaumont", which is one of the few Beaumont queries
    // that has actually converted. The H2s and body copy already say
    // "website design" 12 times, so the old H1 was the odd element out.
    headline: (city) => city.slug === 'beaumont-tx'
      ? 'Website Design in Beaumont, TX That Turns Visitors Into Calls'
  ...
  'google-business-profile': {
    ...
    seo: {
      'houston-tx': {
        title: 'Google Business Profile Optimization Houston TX',
        description: 'Google Business Profile setup and optimization for Houston businesses, including service-area setup for companies without a storefront. From $199.',
      },
      ...
      // Nederland and Port Arthur previously fell through to the generated
      // title, which read "Google Profile in <city>, TX" — a phrase nobody
      // searches (see the shortening removed from buildCityServiceMetadata).
      // Descriptions are deliberately worded differently per city rather than
      // templated, so the three pages do not ship duplicate meta descriptions.
      'nederland-tx': {
        title: 'Google Business Profile Optimization Nederland TX | From $199',
        description: 'Get your Nederland TX business found on Google Maps — correct categories, services, hours, and photos so nearby customers call. One-time setup from $199.',
      },
      'port-arthur-tx': {
        title: 'Google Business Profile Optimization Port Arthur TX | From $199',
        description: 'Google Business Profile help for Port Arthur, TX businesses — categories, services, photos, and reviews that bring in more local calls. Setup from $199.',
      },
    },
...
// A city+service pair that already has a hand-written page must NOT also get a
// generated one: two pages targeting the same city-qualified query split the
// ranking signal and neither wins. That is the cannibalization this site spent
// a round of work undoing for the /digital pages, so Houston is added to the
// generator with its four overlaps held back rather than repeating it.
//
// Keyed by service slug -> the cities whose query is already owned elsewhere.
const cityServiceHasDedicatedPage = {
  'vinyl-banners': ['houston-tx'],   // -> /houston/banners
  'yard-signs': ['houston-tx'],      // -> /houston/yard-signs
  'local-seo': ['houston-tx'],       // -> /houston/local-seo
  'web-development': ['houston-tx'], // -> /houston/web-design
}

// `serviceSlug` is passed explicitly: the objects in cityServiceServices are
// keyed by slug but do not carry one as a property, so it cannot be read off
// the service itself.
export function cityServiceIsAvailable(service, citySlug, serviceSlug) {
  if (service.availableCities && !service.availableCities.includes(citySlug)) return false
  return !(cityServiceHasDedicatedPage[serviceSlug] || []).includes(citySlug)
}

export function getAvailableCityServiceEntries(citySlug) {
  return Object.entries(cityServiceServices).filter(([serviceSlug, service]) =>
    cityServiceIsAvailable(service, citySlug, serviceSlug)
  )
}
...
export function getAvailableCitiesForService(serviceSlug) {
  ...
  return Object.values(cityServiceCities).filter((city) =>
    cityServiceIsAvailable(service, city.slug, serviceSlug)
  )
}
...
/**
 * English path for a city+service pair, or null when that pair is not
 * generated. The Spanish route feeds this into hreflang, and Houston is the
 * case that needs it: /es/area-de-servicio/houston-tx/seo-local exists while
 * /service-area/houston-tx/local-seo does not (the hand-written
 * /houston/local-seo owns that query instead).
 */
export function getEnglishCityServicePath(citySlug, serviceSlug) {
  if (!getCityServiceData(citySlug, serviceSlug)) return null
...
export function getCityServiceData(citySlug, serviceSlug) {
  ...
  if (!cityServiceIsAvailable(service, citySlug, serviceSlug)) return null
