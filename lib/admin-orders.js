...
function compatibleCustomerName(input) {
  return String(input.customerName || input.companyName || "").trim();
}

function orderPayload(input) {
  const payload = {
    order_number: normalizeOrderNumber(input.orderNumber),
    customer_name: compatibleCustomerName(input),
...
function updatePayload(input) {
  ...
  for (const [clientKey, dbKey] of Object.entries(allowed)) {
    if (Object.hasOwn(input, clientKey)) {
      if (clientKey === "orderNumber") payload[dbKey] = normalizeOrderNumber(input[clientKey]);
      else if (clientKey === "customerName") payload[dbKey] = compatibleCustomerName(input);
      else if (clientKey === "projectName") payload[dbKey] = String(input[clientKey] || "").trim();
      else if (clientKey === "customerEmail") payload[dbKey] = normalizeEmail(input[clientKey]) || null;
...
function isMissingCompanyColumn(error) {
  const text = `${error?.message || ""} ${error?.responseText || ""}`.toLowerCase();
  return text.includes("company_name") && text.includes("column");
}

function missingCompanyColumnResult() {
  return {
    ok: false,
    status: 400,
    error:
      "Company cannot be saved yet because Supabase is missing the company_name column. Run: alter table public.orders add column if not exists company_name text;",
  };
}

async function addTimelineUpdate(order, input, fallbackBody) {
  try {
    await supabaseFetch("order_updates", {
      ...
      body: {
        ...
        body: input.updateBody || fallbackBody,
        is_customer_visible: input.isCustomerVisible !== false,
      },
    });
  } catch (error) {
    console.error("Unable to add order timeline update.", error);
  }
}
...
function validateBaseOrder(input) {
  ...
  if (!String(input.companyName || "").trim() && !String(input.customerName || "").trim()) {
    missing.push("company or customer name");
  }
...
export async function createAdminOrder(input) {
  ...
  let created;
  try {
    created = await supabaseFetch("orders?select=*", {
      method: "POST",
      headers: { Prefer: "return=representation" },
      body: orderPayload(input),
    });
  } catch (error) {
    if (isMissingCompanyColumn(error)) return missingCompanyColumnResult();
    throw error;
  }
  const order = created?.[0];

  if (order) {
    await addTimelineUpdate(order, input, "Order was created in Pixel & Panel tracking.");
  }
...
export async function updateAdminOrder(orderNumber, input) {
  ...
  const nextOrderNumber = Object.hasOwn(input, "orderNumber")
    ? normalizeOrderNumber(input.orderNumber)
    : normalizedOrderNumber;
  if (!nextOrderNumber) {
    return { ok: false, status: 400, error: "Order number is required." };
  }
  if (
    Object.hasOwn(input, "companyName") &&
    Object.hasOwn(input, "customerName") &&
    !String(input.companyName || "").trim() &&
    !String(input.customerName || "").trim()
  ) {
    return { ok: false, status: 400, error: "Company or customer name is required." };
  }
  if (Object.hasOwn(input, "projectName") && !String(input.projectName || "").trim()) {
    return { ok: false, status: 400, error: "Project name is required." };
  }
  ...
  const statusChanged = input.status && existing?.status && input.status !== existing.status;
  if (input.updateBody || statusChanged) {
    await addTimelineUpdate(
      order,
      input,
      `Status changed to ${statusLabels[order.status] || order.status}.`,
    );
  }
