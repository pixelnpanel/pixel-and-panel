...
async function getTimelineEventsForOrderIds(orderIds) {
  ...
  throwSupabaseError(error, "Unable to load order timeline events.");
...
async function createInitialTimelineEvent(order, input) {
  ...
  throwSupabaseError(error, "Unable to create initial timeline event.");
}
...
export async function listAdminOrders() {
  ...
  throwSupabaseError(error, "Unable to load orders.");
...
export async function createAdminOrder(input) {
  ...
  const { data: order, error } = await supabase
    .from("orders")
    .insert(baseOrderPayload(input))
    .select("*")
    .single();

  throwSupabaseError(error, "Unable to create order.");
...
export async function updateAdminOrder(orderNumber, input) {
  ...
  const { data: order, error } = await supabase
    .from("orders")
    .update(updatePayload(input, existing.tracking_token))
    .eq("order_number", normalizedOrderNumber)
    .select("*")
    .maybeSingle();

  throwSupabaseError(error, "Unable to update order.");
...
export async function deleteAdminOrder(orderNumber) {
  ...
  throwSupabaseError(error, "Unable to delete order.");
...
export async function addAdminTimelineEvent(orderNumber, input) {
  ...
  throwSupabaseError(error, "Unable to add timeline update.");
  ...
  throwSupabaseError(updateError, "Unable to update order timestamp.");
