...
export async function updateAdminOrderZohoRefs(orderNumber, refs) {
  const normalizedOrderNumber = normalizeOrderNumber(orderNumber);
  if (!normalizedOrderNumber) {
    return { ok: false, status: 400, error: "Order number is required." };
  }
  ...
  const payload = {
    zoho_contact_id: refs.contactId || null,
    zoho_estimate_id: refs.estimateId || null,
    zoho_salesorder_id: refs.salesOrderId || null,
    zoho_document_number: refs.documentNumber || null,
    zoho_status: refs.documentStatus || refs.mode || "synced",
    zoho_last_sync_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  };

  await supabaseFetch(
    `orders?order_number=eq.${encodeURIComponent(normalizedOrderNumber)}`,
    {
      method: "PATCH",
      body: payload,
    },
