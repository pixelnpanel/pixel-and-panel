...
const ciudades = {
  ...
  // Houston se maneja como negocio de área de servicio: no se publica dirección
  // en ninguna parte. Dos servicios no se generan aquí porque ya existe una
  // página propia en /es/houston/* — ver servicioConPaginaPropia.
  'houston-tx': {
    name: 'Houston',
    slug: 'houston-tx',
    enSlug: 'houston-tx',
    // El hub de Houston es /es/houston, no /es/area-de-servicio/houston-tx.
    hubHref: '/es/houston',
    region: 'el área metropolitana de Houston',
    calles: 'Westheimer Road, el Katy Freeway y los corredores comerciales del Beltway 8',
    tipo: 'contratistas, restaurantes, tiendas, consultorios médicos y negocios de servicio industrial',
    oficina: 'la Oficina de Obras Públicas de la Ciudad de Houston',
  },
  'port-arthur-tx': {
    name: 'Port Arthur',
    slug: 'port-arthur-tx',
...
const servicios = {
  'graficos-para-vehiculos': {
    ...
    quoteProduct: 'Rotulación de Vehículos',
    ...
    headline: (city) => `Wraps comerciales y letras para vehículos en ${city.name}, TX`,
    intro: (city) => `Cada milla que manejas por ${city.name} es una oportunidad para que te vean. Pixel & Panel diseña, imprime e instala letras, wraps parciales, imanes y gráficos de flotilla para camionetas, vans, trailers y vehículos de servicio.`,
    localContext: (city) => `En ${city.name}, muchos clientes ven tu vehículo antes de visitar tu sitio web, leer tus reseñas o encontrar tu tarjeta. Los contratistas que trabajan en ${city.calles}, los equipos de servicio que cubren ${city.region} y los repartidores que pasan por toda la ciudad tienen algo en común: el vehículo es la primera impresión. Una camioneta rotulada transmite profesionalismo antes de que toques la puerta. Una flotilla consistente comunica que eres un negocio local establecido, no solo otro vehículo sin marca pasando por la zona.`,
    benefits: [
      'Convierte camionetas y vans de trabajo en publicidad local',
      'Transmite profesionalismo desde el primer contacto',
      'Aumenta reconocimiento de marca en tu área de servicio',
      'Opciones de letras, imanes, wraps parciales o gráficos de flotilla',
    ...
    faqs: (city) => [
      [`¿Cuánto cuesta rotular un vehículo en ${city.name}?`, `Depende del tamaño del vehículo y la cobertura. La rotulación sencilla (nombre, teléfono, sitio web) generalmente empieza en menos de $300. Las envolturas parciales van de $500 a $1,200. Las envolturas completas varían según el vehículo pero generalmente cuestan entre $2,000 y $4,500 instaladas.`],
  ...
  'letreros-para-negocios': {
    ...
    intro: (city) => `Haz que tu ubicación en ${city.name} sea fácil de encontrar y difícil de ignorar. Pixel & Panel diseña y produce letreros de escaparate, letras dimensionales, letras canal iluminadas, displays tipo monumento y paneles exteriores durables adaptados a tu marca.`,
    localContext: (city) => `Los negocios en ${city.name} compiten por atención a lo largo de ${city.calles} — zonas donde los clientes potenciales pasan frente a decenas de opciones antes de decidir dónde detenerse. Un letrero débil o inexistente no solo afecta la visibilidad; también puede hacer que el negocio se vea menos establecido. Para ${city.tipo}, un letrero exterior profesional es una de las piezas de marketing más visibles que tienes. Funciona todos los días, y a diferencia de anuncios digitales, no desaparece cuando se pausa un presupuesto.`,
    benefits: [
      ...
      'Opciones de letras canal, letras dimensionales, monumento, panel e iluminación',
    ],
    cityUses: (city) => [
  ...
  'desarrollo-web': {
    ...
    headline: (city) => `Diseño web para negocios en ${city.name}, TX`,
    intro: (city) => `Tu sitio web es la vitrina digital de tu negocio en ${city.name}. Pixel & Panel crea sitios rápidos, seguros y fáciles de entender que explican tus servicios, apoyan el SEO local y hacen simple que un cliente llame, visite o pida una cotización.`,
    localContext: (city) => `En ${city.name}, la mayoría de los clientes revisan el sitio web de un negocio antes de llamar o visitar, incluso si primero vieron un letrero, una camioneta rotulada, una referencia o una búsqueda en Google. Un sitio lento, desactualizado o difícil de usar en celular puede mandar al cliente a la competencia. Para ${city.tipo}, el sitio no necesita ser complicado. Necesita páginas claras de servicios, señales locales para Google, teléfono visible y un formulario de cotización o contacto que funcione rápido en móvil.`,
    benefits: [
      'Diseño móvil primero para clientes que buscan desde el teléfono',
      'Páginas claras de servicios que responden preguntas antes de la llamada',
      'Formulario de cotización integrado para capturar prospectos',
      'Estructura SEO local incluida desde el inicio',
      'Google, códigos QR y letreros pueden enviar clientes a la página correcta',
    ],
    cityUses: (city) => [
      ...
      `Tiendas locales que necesitan páginas rápidas con horarios, servicios y contacto`,
    ...
    process: [
      ...
      ['Estructura y diseño', 'Diseñamos páginas claras con jerarquía móvil, texto simple y rutas directas para llamar o cotizar.'],
      ['Revisión y ajustes', 'Te mostramos el sitio antes de publicarlo. Hacemos cambios hasta que quedes satisfecho.'],
      ['Publicación y configuración SEO', 'Publicamos el sitio con metadatos locales, estructura para Google y acceso completo.'],
    ],
    faqs: (city) => [
      ...
      [`¿El diseño web ayuda con Google en ${city.name}?`, `Sí. Creamos estructura, metadatos y páginas claras para que Google entienda mejor tus servicios y tu área local. No prometemos posiciones exactas, pero sí una base más fuerte para SEO local.`],
    ],
    crossSell: {
      label: 'SEO local',
...
// Un par ciudad+servicio que ya tiene página propia escrita a mano NO debe
// generarse otra vez aquí: dos páginas compitiendo por la misma búsqueda
// dividen la señal y ninguna gana. Mismo criterio que en la versión en inglés.
//
// Clave: slug del servicio -> ciudades cuya búsqueda ya cubre otra página.
const servicioConPaginaPropia = {
  'banners-de-vinilo': ['houston-tx'],      // -> /es/houston/banners
  'letreros-para-jardin': ['houston-tx'],   // -> /es/houston/letreros-para-jardin
}

export function cityServiceIsAvailableEs(ciudadSlug, servicioSlug) {
  if (!ciudades[ciudadSlug] || !servicios[servicioSlug]) return false
  return !(servicioConPaginaPropia[servicioSlug] || []).includes(ciudadSlug)
}

export const cityServiceStaticParamsEs = Object.keys(ciudades).flatMap((ciudadSlug) =>
  Object.keys(servicios)
    .filter((servicioSlug) => cityServiceIsAvailableEs(ciudadSlug, servicioSlug))
    .map((servicioSlug) => ({ ciudad: ciudadSlug, servicio: servicioSlug }))
)

export function getCityServiceDataEs(ciudadSlug, servicioSlug) {
  if (!cityServiceIsAvailableEs(ciudadSlug, servicioSlug)) return null
  return { city: ciudades[ciudadSlug], service: servicios[servicioSlug] }
}

/**
 * Spanish path for an English city+service pair, or null when that pair has no
 * Spanish page. Returning null matters: it feeds hreflang, and a declaration
 * pointing at a URL that was never generated invalidates the whole pair.
 */
export function getSpanishCityServicePath(enCitySlug, enServiceSlug) {
  ...
  if (!cityServiceIsAvailableEs(city.slug, service.slug)) return null
